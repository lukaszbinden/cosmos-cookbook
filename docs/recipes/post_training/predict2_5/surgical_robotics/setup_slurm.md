...
Use this only if you want to run finetuning via **Slurm** with `scripts/run_finetuning.sh` (multi-node, scheduled jobs). If you have a single server without Slurm, use [run_finetuning_standalone.sh](scripts/run_finetuning_standalone.sh) instead; see [Post-training §4](post_training.md#4-finetuning).
