...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}System Setup for Cosmos Transfer 2.5${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
LARGEST_MOUNT=$(df -h --output=size,target -x tmpfs -x devtmpfs -x squashfs | \
    tail -n +2 | \
    awk '{print $1 " " $2}' | \
    sort -hr | \
    head -1)
...
if [ "$CURRENT_DOCKER_DIR" != "$NEW_DOCKER_DIR" ]; then
    ...
    if [ -d "$CURRENT_DOCKER_DIR" ] && [ "$(ls -A $CURRENT_DOCKER_DIR 2>/dev/null)" ]; then
        ...
        mv "$CURRENT_DOCKER_DIR" "${CURRENT_DOCKER_DIR}.backup.$(date +%Y%m%d_%H%M%S)"
    fi
...
if [ "$CURRENT_CONTAINERD_DIR" != "$NEW_CONTAINERD_DIR" ]; then
    ...
    if [ -d "$CURRENT_CONTAINERD_DIR" ] && [ "$(ls -A $CURRENT_CONTAINERD_DIR 2>/dev/null)" ]; then
        ...
        mv "$CURRENT_CONTAINERD_DIR" "${CURRENT_CONTAINERD_DIR}.backup.$(date +%Y%m%d_%H%M%S)"
    fi
...
# Create symlink from /root/.cache/pip if on different drive
if [ "$LARGEST_PATH" != "/" ]; then
    mkdir -p /root/.cache
